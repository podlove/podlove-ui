...
describe('sharing', () => {
  ...
  it('should generate the correct share link', () => {
    cy.embed('<div></div>', { episode: '/test/episode.json', config: '/test/config.json' }).then(
      app => {
    ...
    cy.select('tab-share--embed--input').should(
      'have.value',
      '<iframe title="Podlove Web Player: Forschergeist - FG066 Klimaneutralität" height="200" src="http://localhost:9000/share.html?config=%2Ftest%2Fconfig.json&episode=%2Ftest%2Fepisode.json" frameborder="0" scrolling="no" tabindex="0"></iframe>'
    )
  })
