<template lang="pug">
  div.flex.px-2.-mx-2.cursor-pointer.rounded-sm(:class="{'font-medium': episode.active}" :style="style" @mouseover="mouseOverHandler" @mouseleave="mouseLeaveHandler")
    span.w-8.py-2.mr-2(v-if="episode.active" @click="play()" aria-hidden="true")
      icon(:type="playing ? 'menu-pause' : 'menu-play'" :size="24")

    span.w-8.py-2.mr-2(v-else @click="select({ index, play: true })" aria-hidden="true")
      icon(v-if="hover" type="menu-play" :size="24")
      span(v-else) {{ index + 1 }}


    span.block.w-full.py-2.mr-2(@click="select({ index, play: true })") {{ episode.title }}

    timer.block.w-18.py-2(v-if="episode.duration" :time="episode.duration")
