...
const speaker = (h, c) =>
  h('span', { class: { speaker: true } }, [
    ...
    c.entry.speaker.avatar
      ? h('img', { class: { 'speaker-avatar': true }, domProps: { src: c.entry.speaker.avatar } })
      : null,
    c.entry.speaker.name
      ? h(
          'span',
          { class: { 'speaker-name': true }, style: c.prerender ? {} : c.speakerTextStyle },
          c.entry.speaker.name
        )
      : null
  ])
...
const highlightText = (h, c, text) => {
  ...
  return text
    ...
    .split('|||')
    .map(text => (text.match(c.query) ? h('span', { class: { highlight: true } }, text) : text))
}
