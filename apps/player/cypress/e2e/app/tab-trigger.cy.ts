...
describe('<tab-trigger>', () => {
  ...
  describe('files', () => {
    it(`shouldn't render when no files are available`, () => {
      cy.app('<tab-trigger tab="files">inner</tab-trigger>');
      cy.query('tab-trigger--files').should('not.exist');
    });

    it('should render when files are available', () => {
      cy.app('<tab-trigger tab="files">inner</tab-trigger>', [audio]);
      cy.query('tab-trigger--files').should('exist');
    });

    it('should emit toggle tab when clicked', (done) => {
      cy.app('<tab-trigger tab="files">inner</tab-trigger>', [audio]).then((app) => {
        onUpdate(app, toggleTab.toString(), (_, { payload }) => {
  ...
  describe('transcripts', () => {
    it(`shouldn't render when no transcripts are available`, () => {
      cy.app('<tab-trigger tab="transcripts">inner</tab-trigger>');
      cy.query('tab-trigger--transcripts').should('not.exist');
    });

    it('should render when transcripts are available', () => {
      cy.app('<tab-trigger tab="transcripts">inner</tab-trigger>', [transcripts]);
      cy.query('tab-trigger--transcripts').should('exist');
    });

    it('should emit toggle tab when clicked', (done) => {
      cy.app('<tab-trigger tab="transcripts">inner</tab-trigger>', [transcripts]).then(
        (app) => {
          onUpdate(app, toggleTab.toString(), (_, { payload }) => {
  ...
  describe('share', () => {
    it('should render',  () => {
      cy.app('<tab-trigger tab="share">inner</tab-trigger>', [reference]);
      cy.query('tab-trigger--share').should('exist');
    });

    it('should emit toggle tab when clicked', (done) => {
      cy.app('<tab-trigger tab="share">inner</tab-trigger>', [reference]).then((app) => {
        onUpdate(app, toggleTab.toString(), (_, { payload }) => {
  ...
  describe('playlist', () => {
    it(`shouldn't render when no playlist are available`, () => {
      cy.app('<tab-trigger tab="playlist">inner</tab-trigger>');
      cy.query('tab-trigger--playlist').should('not.exist');
    });

    it('should render when playlist are available', () => {
      cy.app('<tab-trigger tab="playlist">inner</tab-trigger>', [playlist]);
      cy.query('tab-trigger--playlist').should('exist');
    });

    it('should emit toggle tab when clicked', (done) => {
      cy.app('<tab-trigger tab="playlist">inner</tab-trigger>', [playlist]).then(
        (app) => {
          onUpdate(app, toggleTab.toString(), (_, { payload }) => {
