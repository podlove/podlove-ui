...
describe('<play-button>', () => {
  describe('render', () => {
    it('should render', () => {
      cy.app('<play-button></play-button>').then(() => {
        cy.query('play-button').should('be.visible');
      });
    });

    it('should render the duration', () => {
      cy.app('<play-button></play-button>', [{ duration: '1:11:11' }]).then(() => {
  ...
  describe('interface', () => {
    it('should set the label', () => {
      cy.app('<play-button label="Custom Label"></play-button>').then(() => {
  ...
  describe('logic', () => {
    ...
    beforeEach(() => {
      cy.app('<play-button></play-button>', [episode, audio]).then((app) => {
        assert = onUpdate(app);
