...
describe('<episode-title>', () => {
  describe('render', () => {
    it('should render', () => {
      cy.app('<episode-title></episode-title>', [episode]);
  ...
  describe('logic', () => {
    it('should render just the text if no link is available', () => {
      cy.app('<episode-title></episode-title>', [{ title: episode.title }]);
    ...
    it('should render the target to _blank when in native mode', () => {
      cy.app('<episode-title></episode-title>', [episode, { runtime: { mode: 'embed' } }]);
      cy.query('episode-title--link').should('have.attr', 'target', '_parent');
    });
  });
});
