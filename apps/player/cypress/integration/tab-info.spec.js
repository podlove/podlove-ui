...
const prohibitedMarkup = ['<script>foo</script>', '<iframe />']
...
describe('Info Tab', () => {
  ...
  describe('Episode', () => {
    ...
    describe('Release Date', () => {
      it('renders', function() {
        cy.window().then(setState(this.episode, this.audio, this.show, this.runtime))
        cy.tab('info')
        selectors.tabs.info.episode.meta().contains('11')
        selectors.tabs.info.episode.meta().contains('2')
        selectors.tabs.info.episode.meta().contains('2999')
      })
    ...
    describe('Summary', () => {
      ...
      it(`allows custom markup`, function() {
        this.episode.summary = allowedMarkup
          .map(input => {
  ...
  describe('Show', () => {
    ...
    describe('Summary', () => {
      ...
      it(`allows custom markup`, function() {
        this.show.show.summary = allowedMarkup
          .map(input => {
            if (typeof input === 'string') {
              return input
            }
  ...
  describe('Contributors', () => {
    it('renders a list', function() {
      ...
      selectors.tabs.info
        .contributors()
        .find('li')
        .should('have.length', this.contributors.contributors.length)
    })

    it(`doesn't render if not set`, function() {
      cy.window().then(setState(this.episode, this.audio, this.show, this.runtime))
      cy.tab('info')
      selectors.tabs.info.contributors().should('not.exist')
    })
  })
})
