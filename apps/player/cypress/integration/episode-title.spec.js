...
describe('<episode-title>', () => {
  ...
  describe('logic', () => {
    ...
    it('should render the episode link if available', function () {
      cy.bootstrap('<episode-title></episode-title>', [this.episode])
      cy.select('episode-title--link').its('href').should('be', 'http://link/to/episode')
    })

    it('should render the target to _parent when in native mode', function () {
      cy.bootstrap('<episode-title></episode-title>', [this.episode])
      cy.select('episode-title--link').its('target').should('be', '_parent')
    })

    it('should render the target to _blank when in native mode', function () {
      cy.bootstrap('<episode-title></episode-title>', [this.episode, { runtime: { mode: 'embed' } }])
      cy.select('episode-title--link').its('target').should('be', '_blank')
    })
  })
})
