...
describe('Controls', () => {
  ...
  describe('Play Button', () => {
    it('shows the play button with duration on init', function() {
      cy.window().then(setState(this.episode, this.audio, this.show))
      selectors.controls.playButton.play().contains('00:12')
    })
    ...
    it('shows the restart icon of the player reaches the end', function() {
      cy.window().then(setState(this.episode, this.audio, this.show, { playtime: 11000 }))
      cy.play()
      selectors.controls.playButton.restart()
    })
  })
  ...
  describe('Chapter Buttons', () => {
    ...
    describe('Next Button', () => {
      ...
      it('disables the next chapter button if last chapter is reached', function() {
        cy.window().then(
          setState(this.episode, this.audio, this.show, this.chapters, { playtime: 11000 })
        )
        cy.play()
        cy.pause()
    ...
    describe('Back Button', () => {
      ...
      it('disables the previous chapter button if the first chapter is active', function() {
        cy.window().then(setState(this.episode, this.audio, this.show, this.chapters))
        cy.play()
        cy.pause()
        selectors.chapter.current().contains(this.chapters.chapters[0].title)
