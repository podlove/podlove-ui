...
describe('<tab-chapters>', () => {
  ...
  describe('render', () => {
    ...
    describe('list', () => {
      ...
      it(`should render entries if chapters are available`, function () {
        cy.bootstrap('<tab-chapters style="width: 400px;"></tab-chapters>', [this.theme, this.chapters])
  ...
  describe('logic', () => {
    ...
    beforeEach(function () {
      cy.bootstrap('<tab-chapters style="width: 400px;"></tab-chapters>', [this.theme, this.chapters]).then(app => {
        assert = onUpdate(app)
    ...
    describe('list', () => {
      describe('interactions', () => {
        it('should render a play icon on the active chapter', () => {
          cy.select('tab-chapters--entry').eq(0).find(select('tab-chapters--trigger--menu-play')).should('exist')
        ...
        it('should render a pause icon on the active chapter', () => {
          assert('PLAYER_REQUEST_PLAY', () => {
            cy.select('tab-chapters--entry').eq(0).find(select('tab-chapters--trigger--menu-pause')).should('exist')
          })
        ...
        it('should dispatch the REQUEST_PLAY action on click', done => {
          ...
          cy.select('tab-chapters--entry').eq(0).find(select('tab-chapters--trigger--menu-play')).click()
        })
        ...
        it('should dispatch the REQUEST_PAUSE action on click', done => {
          ...
          cy.select('tab-chapters--entry').eq(0).find(select('tab-chapters--trigger--menu-play')).click()
          cy.select('tab-chapters--entry').eq(0).find(select('tab-chapters--trigger--menu-pause')).click()
        })
        ...
        it('starts the playback on chapter start time', done => {
          ...
          cy.select('tab-chapters--entry').eq(1).find(select('tab-chapters--trigger--menu-play')).click()
        })

        it('should display the link if available', function () {
          cy.select('tab-chapters--entry').eq(2).find('a').should('have.attr', 'href', this.chapters.chapters[2].href)
        })

        it('should show the link icon on interaction', function () {
          cy.select('tab-chapters--entry').eq(2).find('a').trigger('mouseover')
          cy.select('tab-chapters--entry').eq(2).select('tab-chapters--trigger--link').should('exist')
        })
      })

      describe('timers', () => {
        it('should show the remaining time on the active chapter', () => {
          cy.select('tab-chapters--entry').eq(0).find('.timer').should('contain', '-00:06')
        })
