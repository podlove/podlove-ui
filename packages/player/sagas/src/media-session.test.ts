...
describe('MediaSession', () => {
  ...
  beforeEach(() => {
    ...
    selectTitle = vi.fn();
    selectShow = vi.fn();
    selectChapterTitle = vi.fn();
    selectPoster = vi.fn();

    vi.spyOn(helper, 'mediaControl');
  });
  ...
  describe('mediaSessionSaga()', () => {
    ...
    describe('event bridge', () => {
      ...
      beforeEach(() => {
        playEvent = vi.fn();
        pauseEvent = vi.fn();
        stepBackwardEvent = vi.fn();
        stepForwardEvent = vi.fn();
        previousChapterEvent = vi.fn();
        nextChapterEvent = vi.fn();
      ...
      test('should call the previousChapter saga', () => {
        gen.next(nextChapterEvent);
        gen.next();
        gen.next();
        gen.next();
      ...
      test('should call the nextChapter saga', () => {
        gen.next(nextChapterEvent);
        gen.next();
        gen.next();
        gen.next();
