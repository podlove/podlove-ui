...
import { head, findIndex, identity } from 'ramda'

import { loadQuantiles, loadQuantilesPayload, setQuantiles, setQuantilesPayload } from '@podlove/player-actions/quantiles'

const findQuantile = (quantiles: State = [], start: number): number =>
  findIndex((quantile) => head(quantile) === start, quantiles)

const newQuantile = (quantiles: State = [], quantile: [number, number]): State => [...quantiles, quantile]
...
export const selectors = {
  quantiles: identity as (input: State) => State
}
